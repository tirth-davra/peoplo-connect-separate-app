...
function handleCreateSession(ws: any, message: any) {
...
function handleJoinSession(ws: any, message: any) {
...
function handleLeaveSession(ws: any, message: any) {
  const { sessionId, clientId } = message;
...
function handleCleanupSession(ws: any, message: any) {
  const { sessionId, reason } = message;

  const session = sessions.get(sessionId);
  if (!session) {
    console.log(
      "🔌 Session cleanup requested for non-existent session:",
      sessionId
    );
    return;
  }
...
function handleControlMessage(ws: any, message: any) {
  const { sessionId, clientId } = message;
...
function cleanupDisconnectedClient(ws: any) {
...
ipcMain.handle(
  "mouse-click",
  async (event, x: number, y: number, button: string = "left") => {
    try {
...
ipcMain.handle(
  "key-toggle",
  async (event, key: string, down: boolean, modifiers?: string[]) => {
    try {
